...
<body>
    ...
    if(isset($_GET['movie_id']) && isset($_GET['movie_name'])  ):
        if(isset($_POST['enviar'])):
            //Extract data
            $id_reviewform = $_GET['movie_id'];
            $date_reviewform = date('Y/m/d');
            $name_reviewform = $_POST['name'];
            $comment_reviewform = $_POST['comment'];
            $rating_reviewform = $_POST['rating'];

            //Create query to insert 
            $query_form = " INSERT INTO reviews ( review_movie_id, review_date, reviewer_name, review_comment, review_rating) VALUES 
            ( $id_reviewform, '$date_reviewform', ' $name_reviewform', ' $comment_reviewform',  $rating_reviewform)";
            executeQuery($query_form);
            echo '<p>¡Se ha añadido tu nueva reseñas!</p>';
        endif;
        ?>
            ...
            <?php
                ...
                if(!isset($_POST['enviar'])):
                    ...
                    <h3>Deja una reseña:</h3>
                        <form action="<?php echo URL_SITE; ?>details.php?movie_id=<?php echo $_GET['movie_id'] ?>&movie_name=<?php echo $_GET['movie_name'] ?>" method="POST">
                            ...
                            <br/>
                            <label for="rating">Puntua del 1 al 5:</label>
